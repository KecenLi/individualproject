...
此外，自然分布（natural shift）部分在集群上首次运行失败，原因是 **CIFAR‑10.1 未同步到集群数据目录**。这个问题已定位，并改为 **只允许预先下载后提交作业**，避免任务中下载数据导致失败或不合规。
...
其中包含：最新 Phase3 输出、OODRB corruption CSV、以及 OpenOOD/NAC‑UE 的完整分析材料。JPEG 扫描结果将在任务完成后补充同步。
